...
const cardCls = 'bg-gray-800 rounded-xl border border-gray-700 shadow-sm p-5 space-y-4'
    ...
    <div className="p-6 max-w-2xl">
      ...
      <form onSubmit={handleSave} className="space-y-6">
        <div className={cardCls}>
          <h3 className="font-semibold text-white">Genel</h3>
          <div>
            <label className={labelCls}>Varsayılan Model</label>
          ...
          <div>
            <label className={labelCls}>Günlük Video Limiti</label>
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-300">Otomatik Yayın</p>
              <p className="text-xs text-gray-500">Onaylanan konseptleri otomatik yayınla</p>
            </div>
            <button type="button" onClick={() => setSettings(s => ({ ...s, auto_publish: !s.auto_publish }))}
              className={`w-10 h-6 rounded-full transition-colors ${settings.auto_publish ? 'bg-blue-600' : 'bg-gray-600'}`}>
      ...
      <form onSubmit={handlePasswordChange} className="mt-6 space-y-6">
        <div className={cardCls}>
          ...
          <div className="flex items-center gap-3">
            ...
            {pwMsg && (
              <span className={`text-sm ${pwMsg.ok ? 'text-green-400' : 'text-red-400'}`}>
                {pwMsg.text}
              </span>
            )}
          </div>
      ...
      <form onSubmit={handleAddUser} className="mt-6 space-y-6">
        <div className={cardCls}>
          <h3 className="font-semibold text-white">Yeni Admin Kullanıcı Ekle</h3>
          <div>
          ...
          <div>
            <label className={labelCls}>Kullanıcı Adı <span className="text-gray-500">(isteğe bağlı)</span></label>
          ...
          <div className="flex items-center gap-3">
            ...
            {userMsg && (
              <span className={`text-sm ${userMsg.ok ? 'text-green-400' : 'text-red-400'}`}>
                {userMsg.text}
              </span>
            )}
          </div>
